...
```bash
scp build/deb/reqguard_0.3.32_all.deb user@SERVER:/tmp/
ssh user@SERVER
sudo apt install -y /tmp/reqguard_0.3.32_all.deb
```
...
```bash
sudo apt install --reinstall -y /tmp/reqguard_0.3.32_all.deb
```
...
Per la versione `0.3.32` controlla in particolare questa variabile:
...
m                 marca o smarca la riga corrente
a                 marca tutte le righe visibili
z                 svuota la selezione
...
La selezione multipla non dipende da combinazioni speciali del terminale: usa `m` per marcare le righe mentre navighi con `Up/Down`. Le righe selezionate sono marcate con `*`, mostrano `SEL` nella vista principale e sono evidenziate; `b` applica il ban in bulk dalla vista principale, `u` fa unban in bulk dalla vista ban.

Durante ban e unban bulk la TUI mostra `WAITING ...` mentre applica le regole firewall.
...
```text
build/deb/reqguard_0.3.32_all.deb
```
